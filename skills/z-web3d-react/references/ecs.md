...
Koota is an archetype-based ECS (Entity Component System) state management library from the pmndrs ecosystem, optimized for real-time apps running at 60fps+. It uses the term **Trait** instead of "Component" to avoid confusion with React components.
...
The recommended way to modify the world from React event handlers:
...
// UI state (menus, HUD, settings) -- Zustand
...
Koota excels at many entities updated every frame. Zustand excels at simple event-driven UI state. They coexist cleanly.
...
Execution order matters -- call systems in the right sequence inside `useFrame`.
