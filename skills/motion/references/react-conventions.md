...
6. **View Transitions in React**: React's `<ViewTransition>` component layer (shared elements, route transitions, transition types) is owned by the `react-view-transitions` skill, if available — prefer it over hand-rolling `document.startViewTransition` wrappers in React apps.
