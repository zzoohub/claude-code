...
description: |
  Comprehensive security checklists by domain, aligned with OWASP Top 10:2025. Covers authentication, API security, business logic, supply chain, cryptography, SSRF, security misconfiguration, error handling & logging, and AI/LLM security. Use this skill whenever performing a security review, vulnerability assessment, code audit, checking code for OWASP Top 10 risks, or preparing for penetration testing — including partial reviews. Reference specific checklist files based on the domain being reviewed. Also use when the user mentions "security checklist", "vulnerability checklist", "OWASP check", "auth review", "crypto review", "API audit", or any domain-specific security question. Scope is web / server / API and cloud / supply-chain review; mobile-client-internal security (MASVS, jailbreak/root detection) and legal privacy-compliance frameworks (GDPR/CCPA) are out of scope. Do NOT use for: the correctness bugs that survive CI — races, idempotency, cache invalidation, partial failure (use correctness-checklists); design/maintainability smells — coupling, cohesion, leaky abstractions (use maintainability-checklists); style/formatting (linters own those); or implementing the fixes.
---
